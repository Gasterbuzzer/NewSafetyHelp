...
namespace NewSafetyHelp.Callers.IncomingCallWindow
{
    public static class AnimatedCallerPortraitPatches
    {
        ...
        {
            private static readonly MethodInfo TypeText =
                typeof(CallWindowBehavior).GetMethod("TypeText", BindingFlags.NonPublic | BindingFlags.Instance);

            private static readonly FieldInfo TypeRoutine =
                typeof(CallWindowBehavior).GetField("typeRoutine", BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            public static bool Prefix(CallWindowBehavior __instance)
            {
                    ...
                    && !isWarningOrGameOverCaller)
                {
                        ...
                        && currentCaller.CallerHasAnimatedPortrait)
                    {
                        ...
                        if (currentCaller.CallerAnimatedPortraitShouldLoop.HasChanged)
                        {
                            MainCanvasEntry.SetVideoLoop(currentCaller.CallerAnimatedPortraitShouldLoop.Data,
                                MainCanvasEntry.PortraitType.CALLER);
                        }
                    }
                    else
